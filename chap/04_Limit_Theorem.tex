...
\begin{example}
  In order to protect the sovereignty of territorial waters, we prepare to launch 900 missile strikes against enemy targets. Suppose that the number of missiles hitting the target \(X_k\) in \(k\)-th strike follows identical distribution and is independent of each other. If \(\mathbb{E}(X_k) = 4\) and \(\mathrm{Var}(X_k) = 1.21\) \((k = 1, 2, \dots, 900)\), then calculate the probability of the event that the total number of missiles hitting the target is between 3534 and 3699.
\end{example}
\begin{solution}
