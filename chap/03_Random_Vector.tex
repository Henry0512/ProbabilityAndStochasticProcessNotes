...
\begin{proposition}
  ...
  \begin{enumerate}
    ...
    \item \(0 \leq F(x, y) \leq 1\) for all \(x, y \in \R\), and
    \begin{align*}
...
\begin{example}
  ...
  \[
    F(x, y) = A \left(B + \arctan\frac{x}{2}\right) \left(C + \arctan\frac{y}{2}\right), \quad x, y \in \R.
  \]
  \begin{enumerate}
...
\begin{solution}
  \begin{enumerate}
    ...
    \[
      \begin{cases}
        F(-\infty, y) = 0 \implies A (B - \frac{\mpi}{2}) (C + \arctan\frac{y}{2}) = 0, \\
        F(x, -\infty) = 0 \implies A (B + \arctan\frac{x}{2}) (C - \frac{\mpi}{2}) = 0, \\
        F(\infty, \infty) = 1 \implies A (B + \frac{\mpi}{2}) (C + \frac{\mpi}{2}) = 1.
      \end{cases}
    \]
    Solving these equations, we find \(A = \frac{1}{\mpi^2}\), \(B = \frac{\mpi}{2}\), and \(C = \frac{\mpi}{2}\).
    \item We have
    \[
...
\begin{definition}[Joint Probability Density Function]\index{joint probability density function}\index{jointly continuous}
  ...
  \]
  for all \(x, y \in \R\), then \(X\) and \(Y\) are called \emph{jointly continuous}, and the function \(f(x, y)\) is called the \emph{joint probability density function} (joint p.d.f.\@) of \((X, Y)\).
\end{definition}

\begin{proposition}
  For a jointly continuous bivariate random variable \((X, Y)\) with joint p.d.f.\@ \(f(x, y)\), we have
  \begin{enumerate}
    \item \(f(x, y) \geq 0\) for all \(x, y \in \R\).
    \item \(\iint_{\R^2} f(x, y) \, \dd x \, \dd y = 1\).
    ...
    \]
    \item For any \(G \subseteq \R^2\), we have
    \[
      P\{(X, Y) \in G\} = \iint_G f(x, y) \, \dd x \, \dd y.
    \]
  \end{enumerate}
\end{proposition}
...
\begin{definition}[Bivariate Normal Distribution]\index{bivariate normal distribution}
  ...
  \[
    f(x, y) = \frac{1}{2\mpi \sigma_1 \sigma_2 \sqrt{1 - \rho^2}} \me^{-\frac{1}{2(1 - \rho^2)} \left[\frac{(x - \mu_1)^2}{\sigma_1^2} - 2\rho \frac{(x - \mu_1)(y - \mu_2)}{\sigma_1 \sigma_2} + \frac{(y - \mu_2)^2}{\sigma_2^2}\right]}.
  \]
...
\begin{solution}
  \begin{enumerate}
    \item Since \(f(x, y)\) is a joint p.d.f.\@, it must satisfy the property that \(\iint_{\R^2} f(x, y) \, \dd x \, \dd y = 1\). Therefore, we have
    \begin{align*}    
      \iint_{\R^2} f(x, y) \, \dd x \, \dd y &= \int_0^\infty \int_0^\infty A \me^{-(2x + 3y)} \, \dd x \, \dd y \\
      &= A \int_0^\infty \me^{-2x} \, \dd x \int_0^\infty \me^{-3y} \, \dd y \\
      &= A \left(-\frac{1}{2} \me^{-2x}\right)\Big|_0^\infty \left(-\frac{1}{3} \me^{-3y}\right)\Big|_0^\infty = \frac{1}{6} A = 1.
    \end{align*}
    Solving for \(A\), we find \(A = 6\).
    \item We can compute the desired probability as follows:
    \begin{align*}
      P(X < 2, Y < 1) &= \iint_{0 \leq x < 2, 0 \leq y < 1} f(x, y) \, \dd x \, \dd y \\
      &= \int_0^2 \int_0^1 6 \me^{-(2x + 3y)} \, \dd y \, \dd x = 6 \int_0^2 \me^{-2x} \, \dd x \int_0^1 \me^{-3y} \, \dd y \\
      &= 6 \left(-\frac{1}{2} \me^{-2x}\right)\Big|_0^2 \left(-\frac{1}{3} \me^{-3y}\right)\Big|_0^1 = (1 - \me^{-3})(1 - \me^{-4}). \qedhere
    \end{align*}
  \end{enumerate}
\end{solution}
...
\begin{solution}
  \begin{align*}
    P(X + Y \leq 1) &= \iint_{x + y \leq 1} f(x, y) \, \dd x \, \dd y \\
    &= \int_0^\frac{1}{2} \dd x \int_x^{1 - x} \me^{-y} \, \dd y \\
    &= 1 + \me^{-1} - 2e^{-\frac{1}{2}}. \qedhere
  \end{align*}
\end{solution}
...
\begin{align*}
  F_X(x) &= P(X \leq x) = \int_{-\infty}^x \left[\int_{-\infty}^\infty f(u, y) \, \dd y \right] \dd u, \\
  F_Y(y) &= P(Y \leq y) = \int_{-\infty}^y \left[\int_{-\infty}^\infty f(x, v) \, \dd x \right] \dd v.
\end{align*}
...
\begin{solution}
  ...
  \begin{enumerate}
    ...
    \[
      f_X(x) = \int_{-\sqrt{1 - x^2}}^{\sqrt{1 - x^2}} \frac{1}{\mpi} \, \dd y = \frac{2}{\mpi}\sqrt{1 - x^2}.
    \]
  \end{enumerate}
  That is,
  \[
    f_X(x) = \begin{cases}
      \frac{2}{\mpi}\sqrt{1 - x^2}, & -1 \leq x \leq 1, \\
      0, & \text{otherwise}.
    \end{cases} \qedhere
  \]
\end{solution}
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &= \int_{-\infty}^\infty \frac{1}{2\mpi \sigma_1 \sigma_2 \sqrt{1 - \rho^2}} \me^{-\frac{1}{2(1 - \rho^2)} \left[\frac{(x - \mu_1)^2}{\sigma_1^2} + \left(\frac{y - \mu_2}{\sigma_2} - \rho \frac{x - \mu_1}{\sigma_1}\right)^2 - \rho^2 \frac{(x - \mu_1)^2}{\sigma_1^2}\right]} \, \dd y \\
    &= \frac{1}{2\mpi \sigma_1 \sigma_2 \sqrt{1 - \rho^2}} \me^{-\frac{(x - \mu_1)^2}{2\sigma_1^2}} \int_{-\infty}^\infty \me^{-\frac{1}{2(1 - \rho^2)}\left(\frac{y - \mu_2}{\sigma_2} - \rho \frac{x - \mu_1}{\sigma_1}\right)^2} \, \dd y.
  \end{align*}
  Let \(t = \frac{1}{\sqrt{1 - \rho^2}} \left(\frac{y - \mu_2}{\sigma_2} - \rho \frac{x - \mu_1}{\sigma_1}\right)\), then \(\dd t = \frac{1}{\sqrt{1 - \rho^2}} \frac{1}{\sigma_2} \, \dd y\). Therefore,
  \[
    f_X(x) = \frac{1}{2\mpi \sigma_1} \me^{-\frac{(x - \mu_1)^2}{2\sigma_1^2}} \int_{-\infty}^\infty \me^{-\frac{t^2}{2}} \, \dd t = \frac{1}{\sqrt{2\mpi} \sigma_1} \me^{-\frac{(x - \mu_1)^2}{2\sigma_1^2}}.
  \]
...
\begin{definition}[Conditional Distribution Function]\index{conditional distribution function}
  ...
  \[
    F_{X | Y}(x | y) = \lim_{\varepsilon \to 0^+} P\{X \leq x | y - \varepsilon < Y \leq y + \varepsilon\} = \int_{-\infty}^{x} \frac{f(u, y)}{f_Y(y)} \, \dd u
  \]
...
\begin{solution}
  ...
  \[
    f_Y(y) = \int_{-\infty}^\infty f(x, y) \, \dd x = \begin{cases}
      \int_0^y \frac{1}{1 - x} \, \dd x = -\ln(1 - y), & 0 < y < 1, \\
      0, & \text{otherwise}.
    \end{cases} \qedhere
  \]
\end{solution}
...
\begin{solution}
  \begin{enumerate}
    ...
    \[
      f_X(x) = \int_{-\infty}^\infty f(x, y) \, \dd y = \int_0^x 3x \, \dd y = 3x^2, \quad 0 < x < 1,
    \]
    and the marginal p.d.f.\@ of \(Y\) is given by
    \[
      f_Y(y) = \int_{-\infty}^\infty f(x, y) \, \dd x = \int_y^1 3x \, \dd x = \frac{3}{2}(1 - y^2), \quad 0 < y < 1.
    \]
...
\begin{definition}[Independence]\index{independence}
  A bivariate random variable \((X, Y)\) is said to be \emph{independent} if for any \(x, y \in \R\), we have
  \[
...
\begin{proof}
  ...
  \((\Leftarrow)\) Suppose that there exist two sequences of non-negative numbers \(\{p_{i \cdot}\}\) and \(\{p_{\cdot j}\}\) such that \(p_{ij} = p_{i \cdot} p_{\cdot j}\) for all \(i, j\). Then for any \(x, y \in \R\), we have
  \[
...
\begin{proof}
  ...
  \[
    f(x, y) = \frac{\partial^2 F(x, y)}{\partial x \partial y} = \frac{\partial^2 F_X(x) F_Y(y)}{\partial x \partial y} = \frac{\dd F_X(x)}{\dd x} \cdot \frac{\dd F_Y(y)}{\dd y} = f_X(x) f_Y(y).
  \]

  \((\Leftarrow)\) Suppose that \(f(x, y) = f_X(x) f_Y(y)\) at any continuous point of \(f(x, y)\). Then for any \(x, y \in \R\), we have
  \begin{align*}
    F(x, y) &= \int_{-\infty}^x \int_{-\infty}^y f(u, v) \, \dd v \, \dd u = \int_{-\infty}^x \int_{-\infty}^y f_X(u) f_Y(v) \, \dd v \, \dd u \\
    &= \left(\int_{-\infty}^x f_X(u) \, \dd u\right) \left(\int_{-\infty}^y f_Y(v) \, \dd v\right) = F_X(x) F_Y(y). \qedhere
  \end{align*}
\end{proof}
...
\[
  F_Z(z) = P(Z \leq z) = \iint_{x + y \leq z} f(x, y) \, \dd x \, \dd y = \int_{-\infty}^\infty \dd x \int_{-\infty}^{z - x} f(x, y) \, \dd y.
\]
Let \(u = y + x\), then \(\dd u = \dd y\). Therefore,
\[
  F_Z(z) = \int_{-\infty}^\infty \dd x \int_{-\infty}^{z} f(x, u - x) \, \dd u = \int_{-\infty}^z \dd u \int_{-\infty}^\infty f(x, u - x) \, \dd x.
\]
...
\[
  f_Z(z) = \int_{-\infty}^\infty f(z - y, y) \, \dd y.
\]

If \(X\) and \(Y\) are independent, then \(f(x, y) = f_X(x) f_Y(y)\). In this case, the p.d.f.\@ of \(Z\) can be expressed as
\[
  f_Z(z) = \int_{-\infty}^\infty f_X(x) f_Y(z - x) \, \dd x = \int_{-\infty}^\infty f_X(z - y) f_Y(y) \, \dd y.
\]
...
\begin{theorem}
  Let \(X\) and \(Y\) be jointly distributed random variables with p.f.\@ \(p(x, y)\) or p.d.f.\@ \(f(x, y)\). Then the expected value of a function \(h(X, Y)\), denoted by \(\E[h(X, Y)]\), is given by
  \[
    \E[h(X, Y)] = \begin{cases}
      \sum_{x} \sum_{y} h(x, y) p(x, y), & \text{if } (X, Y) \text{ is discrete}, \\
      \int_{-\infty}^\infty \int_{-\infty}^\infty h(x, y) f(x, y) \, \dd x \, \dd y, & \text{if } (X, Y) \text{ is continuous}.
    \end{cases}
  \]
...
\begin{proposition}
  ...
  \begin{enumerate}
    ...
    \item\label{prop:covariance:linear} \(\Cov(aX + b, cY + d) = ac \Cov(X, Y)\) for any constants \(a, b, c\) and \(d\).
...
\begin{proof}[Proof of Property \ref{prop:covariance:linear}]
  ...
  \begin{align*}
    ...
    &= \E[(aX - a\E(X))(cY - c\E(Y))] \\
    &= ac \E[(X - \E(X))(Y - \E(Y))] = ac \Cov(X, Y). \qedhere
  \end{align*}
\end{proof}
...
\begin{solution}
  \begin{align*}
    \E(X) &= \int_{-\infty}^\infty \int_{-\infty}^\infty x f(x, y) \, \dd x \, \dd y = \int_0^1 3x^2 \, \dd x \int_0^x \dd y = \frac{3}{4}, \\
    \E(X^2) &= \int_{-\infty}^\infty \int_{-\infty}^\infty x^2 f(x, y) \, \dd x \, \dd y = \int_0^1 3x^3 \, \dd x \int_0^x \dd y = \frac{3}{5}, \\
    \E(Y) &= \int_{-\infty}^\infty \int_{-\infty}^\infty y f(x, y) \, \dd x \, \dd y = \int_0^1 3x \,\dd x \int_0^x y \, \dd y = \frac{3}{8}, \\
    \E(Y^2) &= \int_{-\infty}^\infty \int_{-\infty}^\infty y^2 f(x, y) \, \dd x \, \dd y = \int_0^1 3x \,\dd x \int_0^x y^2 \, \dd y = \frac{1}{5}, \\
    \E(XY) &= \int_{-\infty}^\infty \int_{-\infty}^\infty xy f(x, y) \, \dd x \, \dd y = \int_0^1 3x^2 \,\dd x \int_0^x y \, \dd y = \frac{3}{10}.
  \end{align*}
  Therefore,
  \begin{align*}
    \Cov(X, Y) &= \E(XY) - \E(X) \E(Y) = \frac{3}{10} - \frac{3}{4} \cdot \frac{3}{8} = \frac{3}{160}, \\
    \Var(X) &= \E(X^2) - (\E(X))^2 = \frac{3}{5} - \left(\frac{3}{4}\right)^2 = \frac{3}{80}, \\
    \Var(Y) &= \E(Y^2) - (\E(Y))^2 = \frac{1}{5} - \left(\frac{3}{8}\right)^2 = \frac{19}{320}.
  \end{align*}
  Thus,
  \[
    \rho(X, Y) = \frac{\Cov(X, Y)}{\sqrt{\Var(X)} \sqrt{\Var(Y)}} = \frac{\frac{3}{160}}{\sqrt{\frac{3}{80}} \sqrt{\frac{19}{320}}} = \frac{3}{\sqrt{57}}. \qedhere
  \]
\end{solution}
...
\begin{proof}
  ...
  \begin{align*}
    \Cov(\xi, \eta) &= \Cov(aX + b, cY + d) = ac \Cov(X, Y), \\
    \Var(\xi) &= \Var(aX + b) = a^2 \Var(X), \\
    \Var(\eta) &= \Var(cY + d) = c^2 \Var(Y).
  \end{align*}
  Therefore,
  \[
    \rho(\xi, \eta) = \frac{\Cov(\xi, \eta)}{\sqrt{\Var(\xi)} \sqrt{\Var(\eta)}} = \frac{ac \Cov(X, Y)}{\sqrt{a^2 \Var(X)} \sqrt{c^2 \Var(Y)}} = \rho(X, Y). \qedhere
  \]
\end{proof}
...
\begin{proof}
  \begin{enumerate}
    ...
    \begin{align*}
      \E(X) &= \int_{-\infty}^\infty \int_{-\infty}^\infty x f(x, y) \, \dd x \, \dd y = \frac{1}{\mpi} \int_{-1}^1 x \, \dd x \int_{-\sqrt{1 - x^2}}^{\sqrt{1 - x^2}} \, \dd y = 0, \\
      \E(X^2) &= \int_{-\infty}^\infty \int_{-\infty}^\infty x^2 f(x, y) \, \dd x \, \dd y = \frac{1}{\mpi} \int_{0}^{2\mpi} \int_{0}^{1} r^3 \cos^2 \theta \, \dd r \, \dd\theta = \frac{1}{4}.
    \end{align*}
    Then,
    \[
      \Var(X) = \E(X^2) - (\E(X))^2 = \frac{1}{4}.
    \]
    By symmetry, we also have \(\E(Y) = 0\), \(\E(Y^2) = \frac{1}{4}\) and \(\Var(Y) = \frac{1}{4}\). Moreover,
    \[
      \E(XY) = \int_{-\infty}^\infty \int_{-\infty}^\infty xy f(x, y) \, \dd x \, \dd y = \frac{1}{\mpi} \int_{0}^{2\mpi} \int_{0}^{1} r^3 \cos \theta \sin \theta \, \dd r \, \dd\theta = 0.
    \]
    Therefore,
    \[
...
\begin{definition}[Central Moment]\index{central moment}
  ...
  \[
    \mu_{n_1, n_2} = \E[(X - \E(X))^{n_1} (Y - \E(Y))^{n_2}].
  \]
\end{definition}
...
\begin{definition}[Covariance Matrix]\index{covariance matrix}
  ...
  \[
    ...
    \end{bmatrix},
  \]
  if all the covariances \(c_{ij} = \Cov(X_i, X_j)\) exist for \(i, j = 1, 2, \dots, n\).
\end{definition}
...
\begin{definition}[Multivariate Normal Distribution]\index{multivariate normal distribution}
  ...
  \[
    f_{\mathbf{X}}(\mathbf{x}) = \frac{1}{(2\mpi)^{\frac{n}{2}} |\Sigma|^{\frac{1}{2}}} \exp\left(-\frac{1}{2} (\mathbf{x} - \mu)^T \Sigma^{-1} (\mathbf{x} - \mu)\right),
  \]
...
\begin{proposition}\label{prop:multivariate-normal-linear-transformation}
  If \(\mathbf{Y} = \mathbf{A}\mathbf{X} + b\), where \(\mathbf{A}\) is an \(n \times n\) non-singular matrix and \(b\) is a (column) \(n\)-dimensional vector of constants, then \(\mathbf{Y} \sim \mathrm{N}(\mathbf{A}\mu + b, \mathbf{A} \Sigma \mathbf{A}^T)\), where \(\mu = \E(\mathbf{X})\) and \(\Sigma\) is the covariance matrix of the multivariate normal random variable \(\mathbf{X}\).
\end{proposition}
...
\begin{solution}
  ...
  \begin{align*}
    \E(Z) &= 2 \E(X) - \E(Y) + 3 = 2 \cdot 1 - 0 + 3 = 5, \\
    \Var(Z) &= 2^2 \Var(X) + (-1)^2 \Var(Y) = 2^2 \cdot 2 + (-1)^2 \cdot 1 = 3^3.
  \end{align*}
  ...
  \[
    f_Z(z) = \frac{1}{3\sqrt{2 \mpi}} \me^{-\frac{(z - 5)^2}{18}}. \qedhere
  \]
\end{solution}

\begin{example}
  Suppose that \(X \sim \mathrm{N}(0, 1)\), \(Y \sim \mathrm{N}(0, 1)\) and \(X\) and \(Y\) are independent. Calculate the value of \(\E(|X - Y|)\).
\end{example}
\begin{solution}
  Let \(Z = X - Y\). Then \(Z \sim \mathrm{N}(0, 2)\), i.e.,
  \[
    f_Z(z) = \frac{1}{2\sqrt{\mpi}} \me^{-\frac{z^2}{4}}.
  \]
  Therefore,
  \[
    \E(|X - Y|) = \E(|Z|) = \int_{-\infty}^\infty |z| \frac{1}{2\sqrt{\mpi}} \me^{-\frac{z^2}{4}} \, \dd z = \frac{1}{\sqrt{\mpi}} \int_0^\infty z \me^{-\frac{z^2}{4}} \, \dd z = \frac{2}{\sqrt{\mpi}}. \qedhere
  \]
\end{solution}
...
\end{example}
\begin{solution}
  Since \((X, Y) \sim \mathrm{N}(0, 1, 3^2, 4^2, -\frac{1}{2})\), we have \(\Var(X) = 3^2\), \(\Var(Y) = 4^2\) and \(\rho(X, Y) = -\frac{1}{2}\). Note that
  \[
    \begin{bmatrix}
      X \\
      Z
  ...
  \begin{align*}
    \Cov(X, Z) &= \Cov\!\left(X, \frac{X}{3} + \frac{Y}{2}\right) = \frac{1}{3} \Cov(X) + \frac{1}{2} \Cov(X, Y) \\
