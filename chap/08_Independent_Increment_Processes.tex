...
\begin{theorem}
  Suppose that \(\{X(t)\}_{t \geq 0}\) is an independent increment process and \(X_0 = 0\). Then the covariance function \(C_X(s, t) = \mathrm{Var}_X[\min(s, t)]\)
\end{theorem}
...
\begin{definition}[Poisson Process]\index{Poisson process}
  ...
  \begin{enumerate}
    ...
    \[
      P\{N_{t + s} - N_s = n\} = e^{-\lambda t} \frac{(\lambda t)^n}{n!}.
    \]
  \end{enumerate}
\end{definition}
...
\begin{proposition}
  ...
  \begin{enumerate}
    ...
    \item \(C_N(s, t) = \mathrm{Var}_N[\min(s, t)] = \lambda \min(s, t)\).
...
\begin{example}
  Let \(\{N_t\}_{t \geq 0}\) be a Poisson process with rate \(\lambda > 0\). Find the values of \(\mathbb{E}[N_t^2]\) and \(\mathbb{E}[(N_t - N_s)^2]\) for all \(0 \leq s < t < \infty\).
\end{example}
\begin{solution}
  Since \(N_t\) follows a Poisson distribution with mean \(\lambda t\), we have
  \[
    \mathbb{E}[N_t^2] = \mathrm{Var}_N[N_t] + (\mathbb{E}[N_t])^2 = \lambda t + (\lambda t)^2.
  \]
  Since \(N_t - N_s\) follows a Poisson distribution with mean \(\lambda (t - s)\), we have
  \[
    \mathbb{E}[(N_t - N_s)^2] = \mathrm{Var}_N[N_t - N_s] + (\mathbb{E}[N_t - N_s])^2 = \lambda (t - s) + (\lambda (t - s))^2. \qedhere
  \]
\end{solution}
