...
\begin{example}
  ...
  \]
  where \(a\) and \(\omega\) are constants, and \(\Theta\) has a uniform distribution in the interval \((0, 2\mpi)\). Since for each fixed \(t = t_1\), \(X_{t_1}\) is a random variable, the collection \(\{X_t\}\) forms a stochastic process. If we select \(\theta_i \in (0, 2\mpi)\) arbitrarily, then
  \[
...
\begin{solution}
  \begin{enumerate}
    ...
    \[
      \E[X_{2.5}] = \E[Y] + 2.5\E[Z] = 0 + 2.5 \cdot 0 = 0,
    \]
    and the variance of \(X_{2.5}\) is
    \[
      \Var(X_{2.5}) = \Var(Y) + (2.5)^2 \Var(Z) = 1 + 6.25 = 7.25.
    \]
    ...
    The mean of \(X_t\) is
    \[
      \E[X_t] = \E[Y] + t\E[Z] = 0 + t \cdot 0 = 0,
    \]
    and the variance of \(X_t\) is
    \[
      \Var(X_t) = \Var(Y) + t^2 \Var(Z) = 1 + t^2.
    \]
    ...
    \[
      \E[X_s] = \E[X_t] = 0, \quad \Var(X_s) = 1 + s^2, \quad \Var(X_t) = 1 + t^2,
    \]
    \[
      \Cov(X_s, X_t) = \E[(Y + Zs)(Y + Zt)] = \E[Y^2] + st\E[Z^2] = 1 + st.
    \]
    Thus,
    \[
...
\begin{definition}[Variance Function]\index{variance function}
  ...
  \[
    \sigma_X^2(t) = \Var(X_t), \quad t \in T.
  \]
\end{definition}
...
\begin{definition}[Autocorrelation Function]\index{autocorrelation function}
  ...
  \[
    R_X(t_1, t_2) = \E[X_{t_1} X_{t_2}], \quad t_1, t_2 \in T.
  \]
\end{definition}
...
\begin{definition}[Autocovariance Function]\index{autocovariance function}
  ...
  \[
    C_X(t_1, t_2) = \Cov(X_{t_1}, X_{t_2}) = \E[(X_{t_1} - \mu_X(t_1))(X_{t_2} - \mu_X(t_2))], \quad t_1, t_2 \in T.
  \]
\end{definition}
...
\begin{solution}
  ...
  \[
    \mu_X(t) = \E[X_t] = \E[Y]\cos\omega t + \E[Z]\sin\omega t = 0,
  \]
  and
  \[
    \E[Y Z] = \E[Y]\E[Z] = 0, \quad \E[Y^2] = \E[Z^2] = \sigma^2.
  \]
  Thus,
  \begin{align*}
    R_X(s, t) &= \E[X_s X_t] \\
...
\begin{example}
  ...
  \]
  where \(a\) and \(\omega\) are constants, and \(\Theta\) has a uniform distribution in the interval \((0, 2\mpi)\). This kind of \(X_t\) is called a sine wave with random phase. Find the mean function \(\mu_X(t)\), the autocorrelation function \(R_X(s, t)\), and the variance function \(\sigma_X^2(t)\) of the process.
\end{example}
\begin{solution}
  The mean function of the process is
  \[
    \mu_X(t) = \E[X_t] = a \E[\sin(\omega t + \Theta)] = 0.
  \]
  The autocorrelation function of the process is
  
  \begin{align*}
    R_X(s, t) &= \E[X_s X_t] \\
    &= a^2 \E[\sin(\omega s + \Theta) \sin(\omega t + \Theta)] \\
    &= a^2 \E\left[\frac{\cos\left(\omega(s - t)\right) - \cos\left(\omega(s + t) + 2\Theta\right)}{2}\right] = \frac{a^2}{2} \cos\left(\omega(s - t)\right).
  \end{align*}
